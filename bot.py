...
def build_app():
    ...
    user_conv = ConversationHandler(
        ...
        states={
            user.SELECT_SERVICE: [
                CallbackQueryHandler(user.service_callback, pattern="^service_")
            ],
            user.ENTER_LINK: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, user.enter_link)
            ],
            user.ENTER_QTY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, user.enter_qty)
            ],
            user.CONFIRM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, user.confirm_order)
            ],
        },
        fallbacks=[]
    )
...
# ❗ BU JOYNI OLIB TASHLAYMIZ – POLLING KERAK EMAS
